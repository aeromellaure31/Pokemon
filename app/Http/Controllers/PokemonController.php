...
class PokemonController extends Controller
{
    ...
    public function retrieveAllUserPokemon(Request $request) {
        $user_pokemon_data = User::where('id', '!=', $request->id)->get();
        $data = [];
        foreach ($user_pokemon_data as $value) {
            $like = explode(',', $value->like);
            $hate = explode(',', $value->hate);
            array_pop($like);
            array_pop($hate);
            $datus = [
            ...
            array_push($data, $datus);
        }
        return response()->json($data);
    }

    public function getUserId(Request $request) {
        $user_pokemon_data = User::select('firstname', 'lastname', 'birthdate', 'username', 'password')->where('id', $request->id)->get();
        return response()->json($user_pokemon_data);
    }
}
